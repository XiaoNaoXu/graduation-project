    ...
    #请求方式为POST
        ...
        if myform.validate_on_submit:
            ...
            if myform.pulish.data:
                ...
                upload_type = request.form.get('selectlabel')
                files = None
                if upload_type == label[1] or upload_type == label[4]:
                    files = request.files.getlist('pluginfile')
                elif upload_type == label[2]:
                    files = request.form.get('pluginfile')
                elif upload_type == label[3]:
                    files = request.files.get('pluginfile')
                print(upload_type, files, label[1], type(files))
                
                plugin_add(main_condict, pluginname, image, inputfilename, inputtype, 
        ...
        if os.path.isdir(path):
            ...
            if filedownload.validate_on_submit:
                ...
                for key in file_list:
                    templist.append(key)
                templist.sort()
                if isexit(sefilecode, templist) == 0 and len(templist) > 0:
                    sefilecode = templist[0]
                if sefilecode != None:
                    try:
                        ext = sefilecode.split('.')[1]
...
def help():
    ...
    if request.method == 'GET':
        ...
        # git_project_url = "git clone https://github.com/XiaoNaoXu/filecopy.git " + main_condict['MoudlesPath'][0] + 'test'
        ...
        return render_template('help.html', temp_dict = temp_dict)
