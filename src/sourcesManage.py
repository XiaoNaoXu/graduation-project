...
def sources_list(sources, main_condist):
    sources_dict = dict()
...
def sources_delete(source, main_condict):
    if os.path.isfile(main_condict['sources_path'][0] + source + '/sources.list'):
        os.remove(main_condict['sources_path'][0] + source + '/sources.list')
        os.rmdir(main_condict['sources_path'][0] + source)

def sources_modification(source_name, new_sources_content, main_condict):
    try:
        f = open(main_condict['sources_path'][0] + source_name + '/sources.list', 'w+')
        f.truncate()
